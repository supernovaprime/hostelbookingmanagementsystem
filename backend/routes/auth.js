...
// Register new user
router.post('/register', authLimiter, securityCheck, [
  body('name').trim().isLength({ min: 2 }).withMessage('Name must be at least 2 characters'),
  body('email').isEmail().normalizeEmail().withMessage('Valid email required'),
  body('password').isLength({ min: 8 }).withMessage('Password must be at least 8 characters'),
  body('role').isIn(['superadmin', 'manager', 'tenant']).withMessage('Invalid role'),
...
router.put('/profile', authenticateToken, [
  ...
  try {
    ...
    const allowedFields = ['name', 'phone', 'dateOfBirth', 'gender', 'address', 'emergencyContact', 'preferences'];
    const updateData = {};
...
router.post('/logout', authenticateToken, async (req, res) => {
  try {
    ...
    // by removing the token. However, we can optionally implement token blacklisting
    // or refresh token revocation here if needed.
...
router.put('/users/:id/deactivate', authenticateToken, requireRole(['superadmin']), async (req, res) => {
  try {
    const user = await User.findByIdAndUpdate(
      req.params.id,
      { isActive: false },
